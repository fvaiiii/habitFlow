...
export default function Stats() {
	...
	if (loading)
		return (
			<div style={{ textAlign: 'center', padding: 50, color: '#8b9a7a' }}>
				Загрузка...
			</div>
		)
	...
	return (
		<div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px' }}>
			...
			>
				...
				>
					<div style={{ fontSize: 32, fontWeight: 600, color: '#5b7a5a' }}>
						{totalHabits}
					</div>
					<div style={{ color: '#b8c8a8', fontSize: 13, marginTop: 8 }}>
						Всего привычек
					</div>
				</div>
				<div
			...
			{maxStreak > 0 && (
				...
				>
					<div style={{ fontSize: 14, color: '#7b8a6b', marginBottom: 8 }}>
						Лучшая серия
					</div>
					<div style={{ fontSize: 48, fontWeight: 600, color: '#5b7a5a' }}>
						{maxStreak} дней
					</div>
				</div>
			)}
				...
				<div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
					{last7Days.map(date => {
						...
						return (
							<div
								key={date}
								style={{ textAlign: 'center', flex: 1, minWidth: 50 }}
							>
								<div
									style={{
										height: 60,
								...
								<div style={{ fontSize: 11, color: '#b8c8a8' }}>
									{date.slice(5)}
								</div>
							</div>
						)
			...
			<div style={{ display: 'grid', gap: 12 }}>
				{habits.map(habit => (
					...
					>
						<div>
							<div
								style={{ fontWeight: 500, color: '#5b7a5a', marginBottom: 4 }}
							>
								{habit.title}
							</div>
						...
						<div style={{ textAlign: 'right' }}>
							<div
								style={{
									fontSize: 24,
									fontWeight: 600,
									color: streaks[habit.id] > 0 ? '#5b7a5a' : '#c4a882',
								}}
							>
								{streaks[habit.id] || 0}
							</div>
